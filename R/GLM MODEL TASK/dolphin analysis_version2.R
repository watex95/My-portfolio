...
str(dolphincomp[,1:4]) #Yes they are binary; all are factors with 2 levels


# 2. Observations to be independent of each other: #check for multicollinearity and drop indpependent variables with high correlation
x=dolphincomp[,c(5:15)] 
library(GGally) 
ggpairs(x) #plot the correlation plot the identify the highly correlated variables
# Above Hg, Cd, and Pb were correlated with atleast one other variable
# they will be excluded in the model

# chack again
y=dolphincomp[,c(6,7,9,11,12,13,14,15)]
ggpairs(y)


# 3. The sample size should be large enough N=(10k/p) k=10 independent variables
# p=smallest proportion of the demographic variables: p=0.25
# Therefore this assumption is not met since N=10*10/0.25 = 400



#GLM model
# Demographic group: Morbillivirus
fitimp <- with(dolimp_long_mids, glm(Morbillivirus~Cu+Se+Tl+Mn+Co+Zn+As+Fe,family = 'binomial'))
...
fitimp2 <- with(dolimp_long_mids, glm(Sex~Cu+Se+Tl+Mn+Co+Zn+As+Fe,family = 'binomial'))
sum_model2=summary(pool(fitimp2))
sum_model2

# Demographic group: Morbillivirus
fitimp3 <- with(dolimp_long_mids, glm(Age_Class~Cu+Se+Tl+Mn+Co+Zn+As+Fe,family = 'binomial'))
sum_model3=summary(pool(fitimp3))
sum_model3

# Demographic group: Location
fitimp4 <- with(dolimp_long_mids, glm(Location~Cu+Se+Tl+Mn+Co+Zn+As+Fe,family = 'binomial'))
